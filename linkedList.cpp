...
void rev_helper(nd* pre, nd* its){
    if(its->nextNode == NULL){
        head = its;
        its->nextNode = pre;
    }
          
    else{
        rev_helper(its, its->nextNode);
        its->nextNode = pre;
    }
